<footer id="footer" data-appear-animation="fadeIn">
	<div class="wrapper grid">
		<div class="unit one-quarter widget widget-about">
			<a id="logo" href="index.html"><img src="{{ asset('public/guest/images/logo-riam.png') }}" alt="" /></a>
			<p style="text-align: justify;">Việc kết nối nhà tuyển dụng và người có nhu cầu việc làm chưa bao giờ là dễ dàng khi có những thông tin trái chiều và chưa kiểm soát chặt chẽ. Chúng tôi có tham vọng đưa người tìm việc và nhà tuyển dụng trực tiếp thấy nhau ở khoảng cách gần hơn. để làm được điều đó chúng tôi cần sự hỗ trợ đóng góp từ mọi người.</p>
		</div>
		
		<div class="unit one-quarter widget widget-contact-us">
			<h4 class="widget-title">Liên hệ</h4>
			<form action="{{ route('post.contact.lien_he')}}" method="post">
				@csrf
				<fieldset>
					<p><input type="text" name="ct_name" required="" value="" placeholder="Họ tên" /></p>
					<p><input type="email" name="ct_email" required="" value="" placeholder="Địa chỉ email" /></p>
					<p><textarea name="ct_content" required="" placeholder="Nội dung"></textarea></p>
					<p><input type="submit" value="Gửi đi" /></p>
				</fieldset>
			</form>
		</div>
		
		<div class="unit one-quarter widget widget-latest-posts">
			<h4 class="widget-title">Địa chỉ</h4>
			<p>Website: riam.edu.vn</p>
			<p>Email: hocvalam24h@gmail.com</p>
			<p>Fax: 08.62 869333</p>
			<p>Điện thoại: 0936 201 222</p>
			<p>Địa chỉ: 181, Lê Đức Thọ, phường 17, quận Gò Vấp, TP.HCM</p>				
		</div>

		<div class="unit one-quarter widget widget-recent-photos">
			<h4 class="widget-title">Liên kết trang</h4>
			<p>Hocvalam.org</p>
			<p>Chungchi.edu.vn</p>
			<p>Dieuduong.edu.vn</p>
			<p>Ranghammat.edu.vn</p>
			<p>Vanthuluutru.org</p>
			<p>Vatlytrilieu.edu.vn</p>
		</div>
	</div>
</footer>

<div id="primary-footer">
	<div class="wrapper grid">
		<div class="unit half">
			Copyright &copy; 2018 Viện Nghiên Cứu Quản Lý Hành Chính. All Rights Reserved 
		</div>
		<div class="unit half">
			<span class="social-icons">
				<a href="javascript:;"><i data-tip-gravity="s" title="Pinterest" class="fa fa-pinterest-square show-tooltip"></i></a> 
				<a href="javascript:;"><i data-tip-gravity="s" title="Facebook" class="fa fa-facebook-square show-tooltip"></i></a> 
				<a href="javascript:;"><i data-tip-gravity="s" title="Dribbble" class="fa fa-dribbble show-tooltip"></i></a> 
				<a href="javascript:;"><i data-tip-gravity="s" title="Linkedin" class="fa fa-linkedin-square show-tooltip"></i></a>
				<a href="javascript:;"><i data-tip-gravity="s" title="Twitter" class="fa fa-twitter-square show-tooltip"></i></a>
			</span>   
		</div>
	</div>
</div>